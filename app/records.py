...
def save_record(
    ...
    *,
    ...
    _CONN.execute(
        '''
        INSERT INTO records (id, type, created_at, updated_at, date, payload, meta)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
            ...
            meta = excluded.meta
        ''',
        (rec_id, record_type, created, now, day, payload_json, meta_json),
    )
    _CONN.commit()
...
def save_questions_record(
    ...
    *,
    ...
    return save_record('questions', payload, meta=meta, record_id=record_id, created_at=created_at, date=date)
